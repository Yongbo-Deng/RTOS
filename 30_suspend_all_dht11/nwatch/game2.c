...
static xTaskHandle g_xTaskHandleCar1;
static xTaskHandle g_xTaskHandleCar2;
static xTaskHandle g_xTaskHandleCar3;
	...
	while (1)
	{
		...
		// if (idata.val == pcar->control_key)
		{
			...
			{	
				...
				// xEventGroupSetBits(g_xEventCar, 0x01);

				xTaskNotifyGive(g_xTaskHandleCar2);
				xTaskNotify(g_xTaskHandleCar3, 10, eSetValueWithOverwrite);
...
static void CarTask2(void *params)
{
	...
	// xSemaphoreTake(g_xSemTicks, portMAX_DELAY);
	// xEventGroupWaitBits(g_xEventCar, 0x01, pdTRUE, pdTRUE, portMAX_DELAY);
	...
	while (1)
	{
		...
		// if (idata.val == pcar->control_key)
		{
			...
			{
				// xSemaphoreGive(g_xSemTicks);
				xEventGroupSetBits(g_xEventCar, (0x01 << 1) );
				vTaskDelete(NULL);
			}
		}
		// else
...
static void CarTask3(void *params)
{
	...
	// xSemaphoreTake(g_xSemTicks, portMAX_DELAY);
	// xEventGroupWaitBits(g_xEventCar, 0x01 | (0x01 << 1), pdTRUE, pdTRUE, portMAX_DELAY);
	do 
	{
		/* Recieve notified value, delay when not recieved, don't verified val */
...
void car_game(void) 
{
	...
	/*	Draw 3 roadmarks. OLED resolution: 128 x 64	*/
	for (i = 0; i < 3; i++)
	{
		for (j = 0; j < 8; j++)
		{
	...
	/*Create 3 car tasks*/
	xTaskCreate(CarTask1, "car1", 128, &g_cars[0], osPriorityNormal + 0, &g_xTaskHandleCar1);
	xTaskCreate(CarTask2, "car2", 128, &g_cars[1], osPriorityNormal + 2, &g_xTaskHandleCar2);
	xTaskCreate(CarTask3, "car3", 128, &g_cars[2], osPriorityNormal + 2, &g_xTaskHandleCar3);
