...
static QueueHandle_t g_xQueueMPU;
...
static void ProcessIRData(void)
{
	...
	if(idata.val == IR_KEY_LEFT) idata.val = UPT_MOVE_LEFT;
	else if(idata.val == IR_KEY_RIGHT) idata.val = UPT_MOVE_RIGHT;
	else if(idata.val == IR_KEY_REPEAT) idata.val = last_val;
...
static void ProcessMPUData(void)
{
	struct input_data idata;
	struct mpu6050_data mdata;
	xQueueReceive(g_xQueueMPU, &mdata, 0);
...
static void InputTask(void *params) {
	...
	while (1)
	{
		...
		if(xQueueHandle)
		{
			...
			else if(xQueueHandle == g_xQueueMPU)
			{
				ProcessMPUData();
			}
		}
...
{		
	...
	xQueueAddToSet(g_xQueueMPU, g_xQueueSetInput);
