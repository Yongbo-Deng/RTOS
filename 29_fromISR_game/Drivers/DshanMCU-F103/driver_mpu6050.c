...
/*Offset according to different devices*/
...
void MPU6050_Task(void *parmas)
{
    ...
    xEventGroupWaitBits(g_xEventMPU6050, 1, pdTRUE, pdFALSE, portMAX_DELAY);

    while (1)
    {
        GetI2C();    //MPU6050 and OLED share the same I2C
        ...
        if (ret == 0)
        {
            ...
            xQueueSend(g_xQueueMPU6050, &result, portMAX_DELAY);
        }
        vTaskDelay(50); 
    }  
}
...
void MPU6050_Callback(void)
{
    ...
    xEventGroupSetBitsFromISR(g_xEventMPU6050, 1, &pxHigherPriorityTaskWoken);
