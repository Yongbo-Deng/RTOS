...
static uint16_t g_xQueues_cnt = 0;
...
#else
	for (uint16_t i = 0; i < g_xQueues_cnt; i++)
	{
		xQueueSendFromISR(g_xQueues[i], pidata, &pxHigherPriorityTaskWoken);
	}
#endif
...
#endif
	g_xQueueIR = xQueueCreate(IR_QUEUE_LEN, sizeof(struct ir_data));	//Queue for paddle
	RegisterQueueHandle(g_xQueueIR);	//Queues for cars
}
...
void RegisterQueueHandle(QueueHandle_t queueHandle)
{
	if(g_xQueues_cnt < 10)
	{
		g_xQueues[g_xQueues_cnt] = queueHandle;
		g_xQueues_cnt++;
	}
}
