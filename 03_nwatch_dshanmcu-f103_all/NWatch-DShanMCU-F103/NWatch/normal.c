...
static display_t draw()
{
	...
	// ÏÔÊ¾ÎÂÊª¶È  100ask
	char temp[8];
	dht11_t * dht11_data_t;
	DHT11_Update();
	dht11_data_t = DHT11_Get_Data();
	
	sprintf((char *)temp,"%dC", dht11_data_t->temp);//
	draw_string(temp, NOINVERT, FRAME_WIDTH-20, FRAME_HEIGHT-8);
	
		
	sprintf((char *)temp,"%d%%", dht11_data_t->hum);//Êª¶È
	draw_string(temp, NOINVERT, FRAME_WIDTH-45, FRAME_HEIGHT-8);
...
static display_t ticker()
{
	...
	millis_t milliseconds = millis_get();
